# Four timed conditions: stringi (plain input), base (plain), and ALTREP
# (charvec) at one and four threads. All four come from one installed charr,
# selected through .charr_backend_environments.
...
# A condition is a backend and a thread count. ALTREP appears twice because
# threading is the only axis on which one backend has two answers; base and
# stringi have no thread count to vary.
conditions <- data.frame(
  condition = c("stringi", "base", "altrep1", "altrep4"),
  condition_label = c("stringi", "base", "ALTREP 1 thread", "ALTREP 4 threads"),
  branch = "main",
  backend = c("stringi", "base", "altrep", "altrep"),
  nthreads = c(1L, 1L, 1L, 4L),
  input_mode = c("plain", "plain", "charvec", "charvec"),
...
if (length(args) >= 1L && identical(args[[1L]], "--dry-run")) {
  ...
  cat(sprintf(
    "%d operations x %d conditions x %d reps = %d timed workers\n",
    length(ops), nrow(conditions), reps,
    length(ops) * nrow(conditions) * reps
  ))
  ...
  cat("conditions: ", paste(conditions$condition, collapse = ", "), "\n",
      sep = "")
  quit(save = "no")
}
...
preflight_worker <- function(op, condition_row, output_path) {
  ...
  process <- run_process(c(
    ...
    shQuote(ops_file),
    shQuote(op),
    condition_row$backend[[1L]],
    condition_row$nthreads[[1L]],
    shQuote(corpus$text),
  ...
  if (!identical(as.integer(fields[[4L]]), condition_row$nthreads[[1L]])) {
    stop("preflight worker used the wrong thread count for ", condition)
  }
}
...
preflight_operation <- function(op) {
  ...
  # agree across all four conditions.
  ...
  cohort_agreement <- length(unique(outputs[conditions$condition != "stringi"])) == 1L
...
summarize_times <- function(data) {
  ...
  do.call(rbind, lapply(complete, function(group) {
    first <- group[1L, c(
      ...
      "input_class", "nthreads", "seed", "corpus_md5",
    "icu_owner", "icu_version",
...
measure <- function(op, condition_row, rep) {
  ...
  if (is.na(seconds) ||
      !identical(as.integer(fields[[4L]]), condition_row$nthreads[[1L]])) {
    stop("worker returned invalid timing metadata")
  }

  package_meta <- metadata[[branch]]
  use_stringi_icu <- identical(condition_row$backend[[1L]], "stringi")
  ...
  data.frame(
    ...
    nthreads = condition_row$nthreads[[1L]],
    rep = rep,
    reps = reps,
    seconds = seconds,
...
cat(sprintf(
  "%s: %d operations, %d plotted conditions, %d fresh-process reps\n",
  label, length(ops), nrow(conditions), reps
))
...
cat(sprintf(
  "\nmeasured %d operations x %d conditions x %d reps\n",
  length(ops), nrow(conditions), reps
))
