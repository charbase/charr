...
test_that("fixed detection matches stringi on unmaterialized inputs", {
  ...
  expect_identical(
    with_altrep(TRUE, str_detect(subject, fixed_pattern)),
    expected
  )
  ...
  expect_identical(
    with_altrep(
      TRUE,
      charr:::ci_detect_fixed(long_subject, long_pattern)
    ),
    with_altrep(
      FALSE,
      charr:::ci_detect_fixed(long_value, long_pattern_value)
    )
  )
...
test_that("fixed detection handles aliases and case-insensitive matching", {
  ...
  expect_identical(
    with_altrep(TRUE, charr:::ci_detect_fixed(alias, alias)),
    with_altrep(
      FALSE,
      charr:::ci_detect_fixed(c("same", "x", "\u00e9"), c("same", "x", "\u00e9"))
    )
  )
...
test_that("fixed detection preserves negate and max_count behavior", {
  ...
  expected <- with_altrep(
    FALSE,
    charr:::ci_detect_fixed(values, "a", max_count = 2L)
  )
  ...
  expect_identical(
    with_altrep(
      TRUE,
      charr:::ci_detect_fixed(subject, pattern, max_count = 2L)
    ),
    expected
  )
  expect_fixed_input_unmaterialized(subject)
  ...
  expected <- with_altrep(
    FALSE,
    charr:::ci_detect_fixed(values, "a", negate = TRUE, max_count = 1L)
  )
...
test_that("fixed detection preserves native recycling warnings", {
  ...
  actual <- NULL
  expect_warning(
    expected <- with_altrep(FALSE, charr:::ci_detect_fixed(values, patterns)),
    warning
  )
  expect_warning(
    actual <- with_altrep(TRUE, charr:::ci_detect_fixed(subject, pattern)),
    warning
  )
...
test_that("empty fixed patterns warn and return missing values", {
  ...
  actual <- NULL
  expect_warning(
    expected <- with_altrep(FALSE, charr:::ci_detect_fixed(values, "")),
    warning
  )
  expect_warning(
    actual <- with_altrep(TRUE, charr:::ci_detect_fixed(subject, pattern)),
...
test_that("zero recycling avoids fixed input readers", {
  ...
  expected <- expect_no_warning(
    with_altrep(
      FALSE,
      charr:::ci_detect_fixed(bytes_value, character())
    )
  )
  actual <- expect_no_warning(
    with_altrep(TRUE, charr:::ci_detect_fixed(subject, pattern))
  )
...
test_that("fixed detection validates subjects before loop shortcuts", {
  ...
  expect_error(
    with_altrep(
      FALSE,
      charr:::ci_detect_fixed(bytes_value, "a", max_count = 0L)
    ),
    "bytes encoding"
  )
  expect_error(
    with_altrep(
      TRUE,
      charr:::ci_detect_fixed(subject, pattern, max_count = 0L)
    ),
    "bytes encoding"
  )
  expect_fixed_input_unmaterialized(subject)
  ...
  expect_error(
    withCallingHandlers(
      with_altrep(FALSE, charr:::ci_detect_fixed(bytes_value, "")),
...
test_that("fixed counting matches stringi on unmaterialized inputs", {
  ...
  expected <- with_altrep(FALSE, str_count(values, fixed(patterns)))
  subject <- charport::as_charvec(values)
  pattern <- charport::as_charvec(patterns)
  fixed_pattern <- fixed(pattern)
  ...
  expect_fixed_input_unmaterialized(fixed_pattern)
  expect_identical(
    with_altrep(TRUE, str_count(subject, fixed_pattern)),
    expected
  )
  expect_fixed_input_unmaterialized(subject)
  expect_fixed_input_unmaterialized(pattern)
...
test_that("fixed counting handles aliases and each matcher lane", {
  ...
  expect_identical(
    with_altrep(TRUE, charr:::ci_count_fixed(shared, shared)),
    with_altrep(FALSE, charr:::ci_count_fixed(values, values))
  )
  ...
  subject <- charport::as_charvec(values)
  pattern <- charport::as_charvec(patterns)
  expected <- with_altrep(FALSE, charr:::ci_count_fixed(values, patterns))

  expect_identical(expected, c(5L, 3L, 2L))
  expect_identical(
    with_altrep(TRUE, charr:::ci_count_fixed(subject, pattern)),
    expected
  )
  expect_fixed_input_unmaterialized(subject)
  expect_fixed_input_unmaterialized(pattern)
...
test_that("fixed counting preserves overlap options", {
  ...
  expected <- with_altrep(
    FALSE,
    charr:::ci_count_fixed(values, patterns, overlap = TRUE)
  )
  expect_identical(expected, c(3L, 2L, 2L))
  expect_identical(
    with_altrep(
      TRUE,
      charr:::ci_count_fixed(subject, pattern, overlap = TRUE)
    ),
    expected
  )
  expect_fixed_input_unmaterialized(subject)
...
test_that("fixed counting preserves native and empty-pattern warnings", {
  ...
  expect_warning(
    actual <- with_altrep(TRUE, charr:::ci_count_fixed(subject, pattern)),
    warning
  )
  expect_identical(actual, expected)
  expect_fixed_input_unmaterialized(subject)
  ...
  expect_warning(
    expected <- with_altrep(FALSE, charr:::ci_count_fixed(values, "")),
    warning
  )
  expect_warning(
    actual <- with_altrep(TRUE, charr:::ci_count_fixed(subject, pattern)),
...
test_that("zero recycling avoids fixed count input readers", {
  ...
  expected <- expect_no_warning(
    with_altrep(FALSE, charr:::ci_count_fixed(bytes_value, character()))
  )
  actual <- expect_no_warning(
    with_altrep(TRUE, charr:::ci_count_fixed(subject, pattern))
  )
...
test_that("fixed counting preserves bytes error order", {
  ...
  expect_error(
    with_altrep(FALSE, charr:::ci_count_fixed(bytes_value, "a")),
    "bytes encoding"
  )
  expect_error(
    with_altrep(TRUE, charr:::ci_count_fixed(subject, pattern)),
  ...
  expect_error(
    with_altrep(FALSE, charr:::ci_count_fixed("abc", bytes_value)),
    "bytes encoding"
  )
  expect_error(
    with_altrep(TRUE, charr:::ci_count_fixed(subject, pattern)),
  ...
  expect_error(
    withCallingHandlers(
      with_altrep(FALSE, charr:::ci_count_fixed(bytes_value, "")),
      warning = function(cnd) {
        reference_warnings <<- reference_warnings + 1L
        invokeRestart("muffleWarning")
      }
...
test_that("fixed counting passes malformed declared UTF-8 through", {
  ...
  expected <- with_altrep(
    FALSE,
    charr:::ci_count_fixed(value, pattern_value)
  )
