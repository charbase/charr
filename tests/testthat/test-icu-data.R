...
#
# ICU data canaries. Each block pins behavior that depends on a retained
# category of the trimmed icudt74l.dat built by tools/trim-icudt.R. The
# stringr suite alone stayed green with CJK segmentation broken, so these
# exist to make silent data degradation loud. The dual run (CHARR_ALTREP
# unset and =true) turns each literal into an equivalence check of both
# backends.
#
# Two tiers, because configure may link the system ICU4C instead of the
# vendored 74.1 bundle (see ../../configure):
#   - unconditional: stable across every ICU charr accepts (>= 61); these
#     must hold on any build, bundled or system.
#   - `if (bundled)`: pinned to ICU 74.1 / CLDR 44 exactly (dictionary
#     segmentations, CLDR orders that drift, warning inventory). On an ICU
#     upgrade of the bundle, re-derive changed literals from installed
#     stringi at the matching ICU version — never hand-tweak; protocol in
#     tools/trim-icudt.R. Plain if(), not skip(), keeps SKIP 0 everywhere.
