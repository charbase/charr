...
test_that("regex predicates consume unmaterialized UTF-16 inputs", {
  ...
  expect_identical(
    with_altrep(TRUE, charr:::ci_detect_regex(subject, pattern)),
    stringi::stri_detect_regex(values, patterns)
  )
  expect_identical(
    with_altrep(TRUE, charr:::ci_count_regex(subject, pattern)),
...
test_that("regex predicates handle exact aliases and recycling", {
  ...
  expect_identical(
    with_altrep(TRUE, charr:::ci_detect_regex(alias, alias)),
    stringi::stri_detect_regex(values, values)
  )
  expect_identical(
    with_altrep(TRUE, charr:::ci_count_regex(alias, alias)),
  ...
  expect_warning(
    actual <- with_altrep(TRUE, charr:::ci_detect_regex(subject, pattern)),
    warning
  )
  ...
  expect_warning(
    actual <- with_altrep(TRUE, charr:::ci_count_regex(subject, pattern)),
    warning
  )
  expect_identical(actual, expected)
...
test_that("regex predicates preserve empty and missing semantics", {
  ...
  expect_warning(
    actual <- with_altrep(
      TRUE, charr:::ci_detect_regex(subject, empty_pattern)
    ),
    warning
  )
  expect_identical(actual, expected)
  ...
  expect_warning(
    actual <- with_altrep(
      TRUE, charr:::ci_count_regex(subject, empty_pattern)
    ),
    warning
  )
  expect_identical(actual, expected)
  ...
  expect_identical(
    with_altrep(TRUE, charr:::ci_detect_regex(anchors, "^$")),
    stringi::stri_detect_regex(c("", "a", NA_character_), "^$")
  )
  expect_identical(
    with_altrep(TRUE, charr:::ci_count_regex(anchors, "^$")),
...
test_that("regex count preserves ICU zero-length advancement", {
  ...
  expect_identical(
    with_altrep(TRUE, charr:::ci_count_regex(subject, "(?=a)")),
...
test_that("regex compilation stays lazy after eager input conversion", {
  ...
  expect_error(
    with_altrep(TRUE, charr:::ci_detect_regex(present, invalid)),
    "U_REGEX_MISSING_CLOSE_BRACKET"
  )
  expect_error(
    with_altrep(TRUE, charr:::ci_count_regex(present, invalid)),
    "U_REGEX_MISSING_CLOSE_BRACKET"
  )
  expect_identical(
    with_altrep(TRUE, charr:::ci_detect_regex(missing, invalid)),
    stringi::stri_detect_regex(NA_character_, "[")
  )
  expect_identical(
    with_altrep(TRUE, charr:::ci_count_regex(missing, invalid)),
    stringi::stri_count_regex(NA_character_, "[")
  )
  expect_identical(
    with_altrep(
      TRUE, charr:::ci_detect_regex(present, invalid, max_count = 0L)
    ),
...
test_that("regex predicates preserve bytes validation and event order", {
  ...
  expect_error(
    with_altrep(TRUE, charr:::ci_detect_regex(bytes_subject, "a")),
    "bytes encoding"
  )
  expect_error(
    with_altrep(TRUE, charr:::ci_count_regex(bytes_subject, "a")),
    "bytes encoding"
  )
  expect_error(
    with_altrep(TRUE, charr:::ci_detect_regex("a", bytes_pattern)),
    "bytes encoding"
  )
  expect_error(
    with_altrep(TRUE, charr:::ci_count_regex("a", bytes_pattern)),
    "bytes encoding"
  )
  expect_error(
    with_altrep(TRUE, charr:::ci_detect_regex(NA_character_, bytes_pattern)),
    "bytes encoding"
  )
  expect_error(
    with_altrep(TRUE, charr:::ci_count_regex(NA_character_, bytes_pattern)),
    "bytes encoding"
  )
  expect_error(
    with_altrep(
      TRUE,
      charr:::ci_detect_regex(bytes_subject, "a", max_count = 0L)
    ),
    "bytes encoding"
  )
  ...
  expect_identical(
    ...
    regex_dc_condition_events(
      with_altrep(
        TRUE, charr:::ci_detect_regex(actual_subject, actual_pattern)
      )
    )
  )
  expect_identical(
    ...
    regex_dc_condition_events(
      with_altrep(
        TRUE, charr:::ci_count_regex(actual_subject, actual_pattern)
      )
    )
  )
