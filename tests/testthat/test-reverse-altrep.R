# Charr-owned tests for the public Reader-backed reversal route.
...
test_that("str_reverse dispatches without materializing its input", {
  ...
  expected <- with_altrep(FALSE, str_reverse(values))
  input <- charport::as_charvec(values)
  result <- with_altrep(TRUE, str_reverse(input))
...
test_that("str_reverse preserves stringi validation and CE_BYTES errors", {
  ...
  expect_error(
    with_altrep(FALSE, str_reverse(malformed_byte)),
    "invalid UTF-8"
  )
  expect_error(with_altrep(TRUE, str_reverse(malformed)), "invalid UTF-8")
  expect_false(charport::charport_info(malformed)$is_materialized)

  bytes <- "\xff"
  Encoding(bytes) <- "bytes"
  expect_error(with_altrep(FALSE, str_reverse(bytes)), "bytes encoding")
  expect_error(
    with_altrep(TRUE, str_reverse(charport::as_charvec(bytes))),
    "bytes encoding"
  )
})
