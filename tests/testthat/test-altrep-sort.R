...
sort_altrep_marked <- function(bytes, encoding) {
...
sort_altrep_events <- function(expr) {
  events <- character()
  tryCatch(
    withCallingHandlers(
      force(expr),
...
test_that("order and rank preserve stable collation and NA placement", {
  ...
  actual_rank <- with_altrep(
    TRUE, charr:::ci_rank(subject, opts_collator = opts)
  )
  expect_identical(
    actual_rank,
  ...
  expect_identical(
    with_altrep(
      TRUE, charr:::ci_order(all_na, na_last = NA, opts_collator = opts)
    ),
    stringi::stri_order(
...
test_that("duplicated preserves collation ties and missing values", {
  ...
  expect_identical(
    with_altrep(TRUE, charr:::ci_order(empty)),
    stringi::stri_order(character())
  )
  expect_identical(
    with_altrep(TRUE, charr:::ci_rank(empty)),
    stringi::stri_rank(character())
  )
  expect_identical(
    with_altrep(TRUE, charr:::ci_duplicated(empty)),
...
test_that("mapped collation helpers preserve marked and malformed input", {
  latin1 <- sort_altrep_marked(c(0x63, 0x61, 0x66, 0xe9), "latin1")
  malformed1 <- sort_altrep_marked(c(0x61, 0xff, 0x62), "UTF-8")
  malformed2 <- sort_altrep_marked(c(0x61, 0xfe, 0x62), "UTF-8")
  values <- c(
  ...
  expect_identical(
    with_altrep(
      TRUE,
      charr:::ci_order(subject, na_last = NA, opts_collator = opts)
    ),
    stringi::stri_order(values, na_last = NA, opts_collator = opts)
  )
  expect_identical(
    with_altrep(TRUE, charr:::ci_rank(subject, opts_collator = opts)),
    stringi::stri_rank(values, opts_collator = opts)
  )
  expect_identical(
    with_altrep(TRUE, charr:::ci_duplicated(subject, opts_collator = opts)),
...
test_that("mapped collation helpers retain validation order", {
  bytes <- sort_altrep_marked(c(0xff, 0xfe), "bytes")
  ...
  actual <- list(
    order = function() with_altrep(TRUE, charr:::ci_order(subject)),
    rank = function() with_altrep(TRUE, charr:::ci_rank(subject)),
    duplicated = function() with_altrep(TRUE, charr:::ci_duplicated(subject))
  )

  for (name in names(expected)) {
    expect_identical(
      sort_altrep_events(expected[[name]]()),
      sort_altrep_events(actual[[name]]()),
      info = name
    )
  }
  ...
  expect_identical(
    ...
    sort_altrep_events(
      with_altrep(
        TRUE, charr:::ci_order(subject, opts_collator = bad_opts)
      )
    )
  )
  expect_sort_source_unmaterialized(subject)
