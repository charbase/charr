...
test_that("fixed starts and ends match stringi on unmaterialized inputs", {
  ...
  expected_start <- with_altrep(FALSE, str_starts(values, fixed(starts)))
  expected_end <- with_altrep(FALSE, str_ends(values, fixed(ends)))
  expect_identical(
    expected_start,
  ...
  expect_identical(
    with_altrep(TRUE, str_starts(subject, fixed_start)),
    expected_start
  )
  expect_identical(
    with_altrep(TRUE, str_ends(subject, fixed_end)),
    expected_end
  )
...
test_that("fixed starts and ends preserve code-point offsets and aliases", {
  ...
  expect_identical(
    with_altrep(TRUE, charr:::ci_startswith_fixed(shared, shared)),
    with_altrep(FALSE, charr:::ci_startswith_fixed(values, values))
  )
  expect_identical(
    with_altrep(TRUE, charr:::ci_endswith_fixed(shared, shared)),
    with_altrep(FALSE, charr:::ci_endswith_fixed(values, values))
  )
  ...
  expected <- with_altrep(
    FALSE,
    charr:::ci_startswith_fixed(value, start_values, from = from)
  )
  expect_identical(expected, c(TRUE, TRUE, TRUE, FALSE, FALSE, TRUE))
  expect_identical(
    with_altrep(
      TRUE,
      charr:::ci_startswith_fixed(subject, pattern, from = from)
    ),
    expected
  )
  expect_position_input_unmaterialized(subject)
  ...
  expected <- with_altrep(
    FALSE,
    charr:::ci_endswith_fixed(value, end_values, to = to)
  )
  expect_identical(expected, c(TRUE, TRUE, TRUE, TRUE, TRUE, FALSE))
  expect_identical(
    with_altrep(
      TRUE,
      charr:::ci_endswith_fixed(subject, pattern, to = to)
    ),
    expected
  )
  expect_position_input_unmaterialized(subject)
  ...
  expect_identical(
    with_altrep(
      TRUE,
      charr:::ci_startswith_fixed(subject, pattern, negate = TRUE)
    ),
    with_altrep(
      FALSE,
      charr:::ci_startswith_fixed(value, "z", negate = TRUE)
    )
  )
  expect_identical(
    with_altrep(
      TRUE,
      charr:::ci_endswith_fixed(subject, pattern, negate = TRUE)
    ),
    with_altrep(
      FALSE,
      charr:::ci_endswith_fixed(value, "z", negate = TRUE)
    )
  )
  expect_position_input_unmaterialized(subject)
  expect_position_input_unmaterialized(pattern)
...
test_that("fixed starts preserve native recycling warnings", {
  ...
  expect_warning(
    expected <- with_altrep(
      FALSE,
      charr:::ci_startswith_fixed(values, patterns)
    ),
    warning
  )
  expect_warning(
    actual <- with_altrep(
      TRUE,
      charr:::ci_startswith_fixed(subject, pattern)
    ),
    warning
  )
...
test_that("fixed locate first preserves code-point positions", {
  ...
  expected_first <- with_altrep(
    FALSE,
    charr:::ci_locate_first_fixed(values, patterns, get_length = TRUE)
  )
  actual_first <- with_altrep(
    TRUE,
    charr:::ci_locate_first_fixed(subject, pattern, get_length = TRUE)
  )
  expect_identical(actual_first, expected_first)
  ...
  expect_identical(
    with_altrep(TRUE, charr:::ci_locate_first_fixed(shared, shared)),
    with_altrep(
      FALSE,
      charr:::ci_locate_first_fixed(shared_values, shared_values)
    )
  )
  expect_position_input_unmaterialized(shared)
})
...
test_that("fixed locate all preserves overlap and no-match shapes", {
  ...
  expected <- with_altrep(
    FALSE,
    charr:::ci_locate_all_fixed(values, patterns, overlap = TRUE)
  )
  actual <- with_altrep(
    TRUE,
    charr:::ci_locate_all_fixed(subject, pattern, overlap = TRUE)
  )
  expect_identical(actual, expected)
  ...
  shared <- charport::as_charvec(shared_values)
  expect_identical(
    with_altrep(TRUE, charr:::ci_locate_all_fixed(shared, shared)),
    with_altrep(
      FALSE,
      charr:::ci_locate_all_fixed(shared_values, shared_values)
    )
  )
  expect_position_input_unmaterialized(shared)
})
...
test_that("fixed locate preserves recycling and lenient UTF-8 indexing", {
  ...
  expect_warning(
    expected <- with_altrep(
      FALSE,
      charr:::ci_locate_first_fixed(values, patterns)
    ),
    warning
  )
  expect_warning(
    actual <- with_altrep(
      TRUE,
      charr:::ci_locate_first_fixed(subject, pattern)
    ),
    warning
  )
  expect_identical(actual, expected)
  ...
  expect_identical(
    with_altrep(TRUE, charr:::ci_locate_first_fixed(subject, pattern)),
    with_altrep(FALSE, charr:::ci_locate_first_fixed(value, "b"))
  )
  expect_identical(
    with_altrep(TRUE, charr:::ci_locate_all_fixed(subject, pattern)),
    with_altrep(FALSE, charr:::ci_locate_all_fixed(value, "b"))
  )
  expect_position_input_unmaterialized(subject)
  expect_position_input_unmaterialized(pattern)
})
...
test_that("empty fixed position patterns warn and preserve NA shapes", {
  ...
  expect_warning(
    expected <- with_altrep(FALSE, charr:::ci_startswith_fixed(values, "")),
    warning
  )
  expect_warning(
    actual <- with_altrep(TRUE, charr:::ci_startswith_fixed(subject, pattern)),
    warning
  )
  expect_identical(actual, expected)
  ...
  expect_warning(
    expected <- with_altrep(
      FALSE,
      charr:::ci_locate_all_fixed(values, "", omit_no_match = TRUE)
    ),
    warning
  )
  expect_warning(
...
test_that("zero recycling avoids fixed position input readers", {
  ...
  expect_identical(
    expect_no_warning(
      with_altrep(
        TRUE,
        charr:::ci_startswith_fixed(subject, pattern, from = integer())
      )
    ),
    logical()
  )
  expect_identical(
    expect_no_warning(
      with_altrep(TRUE, charr:::ci_endswith_fixed(subject, empty_pattern))
    ),
    logical()
  )
  expect_identical(
    expect_no_warning(
      with_altrep(TRUE, charr:::ci_locate_first_fixed(subject, empty_pattern))
    ),
    cbind(start = integer(), end = integer())
  )
  expect_identical(
    expect_no_warning(
      with_altrep(TRUE, charr:::ci_locate_all_fixed(subject, empty_pattern))
    ),
    list()
  )
  expect_position_input_unmaterialized(subject)
...
test_that("fixed position operations preserve bytes error order", {
  ...
  expect_error(
    with_altrep(TRUE, charr:::ci_startswith_fixed(subject, pattern)),
    "bytes encoding"
  )
  expect_error(
    with_altrep(TRUE, charr:::ci_endswith_fixed(subject, pattern)),
    "bytes encoding"
  )
  expect_error(
    with_altrep(TRUE, charr:::ci_locate_first_fixed(subject, pattern)),
