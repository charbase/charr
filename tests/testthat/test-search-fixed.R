...
#
# Fixed-search encoding, recycling, and option equivalence. The OFF
# route exercises installed stringi; the ON route exercises the Reader-backed
# kernels. Every literal must hold identically on both routes -- fixed
# byte-search does NOT validate UTF-8 (it matches raw declared-UTF-8 bytes
# verbatim, like StriContainerUTF8), so malformed UTF-8 is passed through, not
# rejected; only bytes-marked input errors.
...
detect_fixed <- function(...) charr:::ci_detect_fixed(...)
count_fixed <- function(...) charr:::ci_count_fixed(...)
starts_fixed <- function(...) charr:::ci_startswith_fixed(...)
ends_fixed <- function(...) charr:::ci_endswith_fixed(...)
...
test_that("fixed search rejects bytes-marked input and patterns", {
  ...
  # StriContainerUTF8 acquires all str elements before detect's max_count
...
test_that("fixed search passes malformed UTF-8 through verbatim (no validation)", {
  ...
  # bytes and never rejects or replaces malformed sequences (StriContainerUTF8
  # passes IS_UTF8 through verbatim, killbom, no validation). These therefore
...
test_that("fixed matchers stay within Reader-backed record lengths", {
  ...
  # preceding record. The OFF route asserts the same observable result.
