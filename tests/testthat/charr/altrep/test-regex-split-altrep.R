...
split_regex <- function(...) charr:::ci_split_regex(...)


test_that("regex split delimits UTF-16 matches, fields re-encoded to UTF-8", {
  strings <- charr:::ci_trim_both(c(" a,😀,𐐷 ", " x1y22z ", " none ", NA))
...
test_that("regex split honours n, omit_empty, tokens_only", {
  strings <- charr:::ci_replace_all_fixed(c("a,,b,c", ",a,"), "~", "~")
  expect_identical(charport::is_charvec(strings), charr_altrep())
  ...
  expect_identical(
    split_regex(charr:::ci_replace_all_fixed("a,,b", "~", "~"), ",", omit_empty = NA),
    list(c("a", NA, "b"))
  )
})
...
test_that("regex split simplify builds a padded matrix; zero-length matches", {
  strings <- charr:::ci_replace_all_fixed(c("a,b,c", "x,y"), "~", "~")
  expect_identical(charport::is_charvec(strings), charr_altrep())
  ...
  expect_identical(
    split_regex(charr:::ci_replace_all_fixed("abc", "~", "~"), "(?=.)"),
    list(c("", "a", "b", "c"))
  )
})
