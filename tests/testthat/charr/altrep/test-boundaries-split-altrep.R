# charr-owned targeted equivalence tests for Reader/Builder boundary split.

split_boundaries <- function(...) charr:::ci_split_boundaries(...)

boundary_split_charvec <- function(x) charr::str_trim(x)


test_that("boundary split emits every ICU iterator type through Builders", {
  strings <- boundary_split_charvec(c(
...
test_that("boundary split preserves n, tokens_only, and skip accounting", {
  string <- boundary_split_charvec("abc 123 日本語 カナ")
...
test_that("boundary split preserves recycling, NA, empty, and simplify shapes", {
  strings <- boundary_split_charvec(c("abc def", "", NA, "123 456"))
  expect_identical(charport::is_charvec(strings), charr_altrep())
...
test_that("boundary split retains copied iterator-open and n-error order", {
  empty <- boundary_split_charvec("")
  text <- boundary_split_charvec(c("abc", "def"))
  bad_rules <- list(type = "[")
  expect_identical(
...
test_that("boundary split supports custom rules and malformed UTF-8", {
  ...
  strings <- boundary_split_charvec(c("abc 123", "é42"))
  expect_identical(charport::is_charvec(strings), charr_altrep())
  expect_identical(
