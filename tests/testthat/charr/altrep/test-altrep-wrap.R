...
wrap_altrep_call <- function(on, string, ...) {
  with_altrep(
    on,
    charr:::ci_wrap(
...
wrap_altrep_marked <- function(bytes, encoding) {
  value <- rawToChar(as.raw(bytes))
  Encoding(value) <- encoding
  value
}

expect_wrap_altrep_unmaterialized <- function(x) {
  expect_true(charport::is_charvec(x))
  expect_false(charport::charport_info(x)$is_materialized)
}

wrap_altrep_events <- function(on, string, ...) {
  events <- character()
  tryCatch(
    withCallingHandlers(
      wrap_altrep_call(on, string, ...),
...
wrap_altrep_event_types <- function(events) {
  sub(":.*$", "", events)
}

wrap_altrep_normalize_events <- function(events) {
...
test_that("wrap preserves serial line breaking and list shape", {
  ...
  for (cost_exponent in c(-1, 2)) {
    for (use_length in c(FALSE, TRUE)) {
      actual <- suppressWarnings(wrap_altrep_call(
      ...
      expected <- suppressWarnings(wrap_altrep_call(
      ...
      expect_true(all(vapply(
        actual, charport::is_charvec, logical(1L)
      )))
      expect_true(all(vapply(
        actual,
  ...
  expect_wrap_altrep_unmaterialized(subject)
  expect_wrap_altrep_unmaterialized(prefix)
  expect_wrap_altrep_unmaterialized(initial)
})
...
test_that("wrap reuses exact aliases and consumes its lazy line output", {
  ...
  expect_wrap_altrep_unmaterialized(first[[1L]])
  expect_wrap_altrep_unmaterialized(shared)

  second <- wrap_altrep_call(
    TRUE, first[[1L]], width = 6L, cost_exponent = -1,
    whitespace_only = FALSE, use_length = TRUE
  )
  expected_second <- wrap_altrep_call(
    FALSE, expected_first[[1L]], width = 6L, cost_exponent = -1,
    whitespace_only = FALSE, use_length = TRUE
  )

  expect_true(all(vapply(
    second, charport::is_charvec, logical(1L)
  )))
  expect_wrap_altrep_unmaterialized(first[[1L]])
...
test_that("wrap preserves encoding normalization and large staged output", {
  latin1 <- wrap_altrep_marked(c(0x63, 0x61, 0x66, 0xe9), "latin1")
  ...
  expect_wrap_altrep_unmaterialized(large_subject)
})

test_that("wrap preserves missing, empty, bytes, and malformed behavior", {
  values <- c("", " ", NA_character_, "one two")
  subject <- charport::as_charvec(values)
  actual <- wrap_altrep_call(TRUE, subject, width = 4L)
  expected <- wrap_altrep_call(FALSE, values, width = 4L)
  ...
  expect_identical(wrap_altrep_call(TRUE, empty), list())
  expect_wrap_altrep_unmaterialized(empty)

  bytes <- wrap_altrep_marked(
    c(0x6f, 0x6e, 0x65, 0x20, 0xff), "bytes"
  )
  bytes_subject <- charport::as_charvec(bytes)
  expect_identical(
    wrap_altrep_events(TRUE, bytes_subject),
    wrap_altrep_events(FALSE, bytes)
  )
  expect_wrap_altrep_unmaterialized(bytes_subject)

  malformed <- wrap_altrep_marked(c(0x61, 0xc3, 0x28, 0x62), "UTF-8")
  malformed_subject <- charport::as_charvec(malformed)
  expect_identical(
    wrap_altrep_events(TRUE, malformed_subject, width = 2L),
    wrap_altrep_normalize_events(
      wrap_altrep_events(FALSE, malformed, width = 2L)
    )
  )
  expect_wrap_altrep_unmaterialized(malformed_subject)
})

test_that("wrap preserves locale-warning and argument-error order", {
  newline <- "alpha\nbeta"
  subject <- charport::as_charvec(newline)
  actual <- wrap_altrep_events(
    TRUE, subject, width = 4L, locale = "zz_ZZ"
  )
  expected <- wrap_altrep_events(
    FALSE, newline, width = 4L, locale = "zz_ZZ"
  )

  expect_identical(wrap_altrep_event_types(actual), c("warning", "error"))
  expect_identical(actual, expected)
  expect_wrap_altrep_unmaterialized(subject)

  option_actual <- wrap_altrep_events(
    TRUE, subject, indent = -1L, locale = "zz_ZZ"
  )
  option_expected <- wrap_altrep_events(
    FALSE, newline, indent = -1L, locale = "zz_ZZ"
  )
  expect_identical(wrap_altrep_event_types(option_actual), "error")
