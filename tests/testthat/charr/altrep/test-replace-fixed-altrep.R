test_that("fixed replacement preserves byte-search semantics", {
  ...
  replaced <- charr:::ci_replace_all_fixed(malformed, "a", "x")
  expect_equal(charToRaw(replaced), as.raw(c(0x78, 0xff, 0x62)))
  expect_equal(
    charToRaw(charr:::ci_replace_first_fixed("a", "a", malformed)),
    as.raw(c(0x61, 0xff, 0x62))
  )
  expect_equal(
    charr:::ci_replace_first_fixed("a", "z", NA_character_),
    "a"
  )
  expect_true(is.na(
    charr:::ci_replace_first_fixed("a", "a", NA_character_)
  ))
  expect_warning(
    expect_true(is.na(charr:::ci_replace_all_fixed("a", "", "x"))),
    "empty search patterns"
  )
  expect_error(
    charr:::ci_replace_first_fixed(bytes, "z", "x"),
    "bytes encoding"
  )
  expect_error(
    charr:::ci_replace_first_fixed("a", "z", bytes),
    "bytes encoding"
  )
})
...
test_that("fixed replacement sequential mode preserves copied order", {
  ...
  run <- function() {
    list(
      first = charr:::ci_replace_first_fixed(strings, "a", "X"),
      all = charr:::ci_replace_all_fixed(strings, "a", "X"),
      sequential = charr:::ci_replace_all_fixed(
...
test_that("fixed replacement borrows stable values and owns conversions", {
  ...
  stable <- charr:::ci_replace_all_fixed(large, "~", "~")
  expect_identical(charport::is_charvec(stable), charr_altrep())
  expect_identical(
    charr:::ci_replace_all_fixed("a-a", "a", stable),
    paste0(stable, "-", stable)
  )

  latin1 <- rawToChar(as.raw(0xe9))
  Encoding(latin1) <- "latin1"
  converted <- charr:::ci_replace_all_fixed("a-a", "a", latin1)
