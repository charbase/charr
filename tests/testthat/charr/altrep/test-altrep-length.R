...
test_that("length and width read an unmaterialized charvec", {
  ...
  expected_length <- with_altrep(FALSE, str_length(values))
  expected_width <- with_altrep(FALSE, str_width(values))
  input <- charport::as_charvec(values)

  expect_length_input_unmaterialized(input)
  expect_identical(
    with_altrep(TRUE, str_length(input)),
    expected_length
  )
  expect_length_input_unmaterialized(input)
  expect_identical(
    with_altrep(TRUE, str_width(input)),
    expected_width
  )
  expect_length_input_unmaterialized(input)
})
...
test_that("empty charvec inputs avoid materialization", {
  ...
  expect_identical(with_altrep(TRUE, str_length(input)), integer())
  expect_identical(with_altrep(TRUE, str_width(input)), integer())
  expect_length_input_unmaterialized(input)
})

test_that("length counts a leading BOM while width ignores it", {
  value <- enc2utf8("\ufeffa")
  expected_length <- with_altrep(FALSE, str_length(value))
  expected_width <- with_altrep(FALSE, str_width(value))
  input <- charport::as_charvec(value)

  expect_identical(expected_length, 2L)
  expect_identical(with_altrep(TRUE, str_length(input)), expected_length)
  expect_length_input_unmaterialized(input)
  expect_identical(expected_width, 1L)
  expect_identical(with_altrep(TRUE, str_width(input)), expected_width)
  expect_length_input_unmaterialized(input)
})
...
test_that("length and width reject bytes and invalid UTF-8", {
  ...
  expect_error(with_altrep(FALSE, str_length(bytes_value)), "bytes encoding")
  expect_error(with_altrep(TRUE, str_length(bytes)), "bytes encoding")
  expect_length_input_unmaterialized(bytes)
  expect_error(with_altrep(FALSE, str_width(bytes_value)), "bytes encoding")
  expect_error(with_altrep(TRUE, str_width(bytes)), "bytes encoding")
  expect_length_input_unmaterialized(bytes)

  expect_error(
    with_altrep(FALSE, str_length(malformed_value)),
    "invalid UTF-8"
  )
  expect_error(with_altrep(TRUE, str_length(malformed)), "invalid UTF-8")
  expect_length_input_unmaterialized(malformed)
  expect_error(
    with_altrep(FALSE, str_width(malformed_value)),
    "invalid UTF-8"
  )
  expect_error(with_altrep(TRUE, str_width(malformed)), "invalid UTF-8")
  expect_length_input_unmaterialized(malformed)
})
...
test_that("length trusts ASCII canonicalization but rejects real bytes", {
  ...
  expect_identical(with_altrep(FALSE, str_length(ascii)), 3L)
  expect_identical(with_altrep(TRUE, str_length(ascii_input)), 3L)
  ...
  expect_error(
    with_altrep(FALSE, str_length(utf8_bytes)),
    "bytes encoding"
  )
  expect_error(
    with_altrep(TRUE, str_length(utf8_bytes_input)),
    "bytes encoding"
  )
...
test_that("length validates UTF-8 boundary sequences", {
  ...
  for (bytes in valid) {
    ...
    input <- charport::as_charvec(value)
    expected <- with_altrep(FALSE, str_length(value))

    expect_identical(expected, 1L)
    expect_identical(with_altrep(TRUE, str_length(input)), expected)
    expect_length_input_unmaterialized(input)
  }
  ...
  for (bytes in invalid) {
    ...
    expect_error(with_altrep(FALSE, str_length(value)), "invalid UTF-8")
    expect_error(with_altrep(TRUE, str_length(input)), "invalid UTF-8")
    expect_length_input_unmaterialized(input)
  }
})
...
test_that("length handles large vectors and invalid elements at any position", {
  ...
  expect_identical(with_altrep(FALSE, str_length(values)), expected)
  expect_identical(with_altrep(TRUE, str_length(subject)), expected)
