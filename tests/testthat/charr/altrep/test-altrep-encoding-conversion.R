...
test_that("marked encode preserves converters, marks, and raw output", {
  ...
  for (to in c("UTF-8", "ISO-8859-1", "UTF-7")) {
    actual <- encoding_conversion_events(
      with_altrep(TRUE, charr:::ci_encode(subject, NULL, to, FALSE))
    )
  ...
  actual_raw <- encoding_conversion_events(
    with_altrep(
      TRUE, charr:::ci_encode(subject, NULL, "UTF-16LE", TRUE)
    )
  )
  ...
  actual_bytes <- encoding_conversion_events(
    with_altrep(
      TRUE, charr:::ci_encode(bytes_subject, NULL, "UTF-8", FALSE)
    )
  )
...
test_that("explicit encode preserves raw, list-raw, and character byte input", {
  ...
  actual <- with_altrep(
    TRUE, charr:::ci_encode(input, "UTF-8", "UTF-8", FALSE)
  )
  ...
  actual_raw <- with_altrep(
    TRUE, charr:::ci_encode(input, "UTF-8", "UTF-16LE", TRUE)
  )
  expect_identical(
    actual_raw,
  ...
  single <- with_altrep(
    TRUE, charr:::ci_encode(single_raw, "UTF-8", "UTF-8", FALSE)
  )
  ...
  explicit <- with_altrep(
    TRUE,
    charr:::ci_encode(marked_subject, "ISO-8859-1", "UTF-8", FALSE)
  )
...
test_that("converter warnings retain count and source-before-target order", {
  ...
  actual <- encoding_conversion_events(
    with_altrep(
      TRUE, charr:::ci_encode(input, "UTF-8", "ISO-8859-1", FALSE)
    )
  )
  expected <- encoding_conversion_events(
...
test_that("converter callbacks release Reader and ICU state before warn=2", {
  ...
  expect_error(
    with_altrep(
      TRUE,
      charr:::ci_encode(subject, "UTF-8", "ISO-8859-1", FALSE)
    ),
    "Unicode code point",
    fixed = TRUE
  )
  expect_identical(
    with_altrep(
      TRUE, charr:::ci_encode(subject, "UTF-8", "UTF-8", FALSE)
    ),
    "\U0001f642"
  )
  expect_encoding_conversion_unmaterialized(subject)
...
test_that("encoding conversion keeps each encode NUL policy", {
  marked <- encoding_conversion_events(
    with_altrep(
      TRUE, charr:::ci_encode("A", NULL, "UTF-16LE", FALSE)
    )
  )
  ...
  explicit <- encoding_conversion_events(
    with_altrep(
      TRUE,
      charr:::ci_encode(as.raw(0), "UTF-8", "UTF-8", FALSE)
    )
  )
  ...
  expect_identical(
    with_altrep(
      TRUE, charr:::ci_encode("A", NULL, "UTF-16LE", TRUE)
    ),
    stringi::stri_encode("A", NULL, "UTF-16LE", TRUE)
  )
  expect_identical(
    with_altrep(
      TRUE, charr:::ci_encode(as.raw(0), "UTF-8", "UTF-8", TRUE)
    ),
...
test_that("encode preserves all zero-length shapes", {
  ...
  marked <- with_altrep(
    TRUE, charr:::ci_encode(empty, NULL, "not-an-encoding", FALSE)
  )
  explicit <- with_altrep(
    TRUE,
    charr:::ci_encode(
  ...
  expect_identical(
    with_altrep(
      TRUE, charr:::ci_encode(raw(), "UTF-8", "UTF-8", FALSE)
    ),
    ""
  )
  expect_identical(
    with_altrep(
      TRUE, charr:::ci_encode(NULL, "UTF-8", "UTF-8", FALSE)
    ),
    NA_character_
  )
