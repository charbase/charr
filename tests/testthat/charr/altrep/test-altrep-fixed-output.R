...
test_that("fixed first extraction uses unmaterialized inputs and outputs", {
  ...
  first <- with_altrep(TRUE, charr:::ci_extract_first_fixed(subject, pattern))
  expect_true(charport::is_charvec(first))
  expect_false(charport::charport_info(first)$is_materialized)
  expect_identical(
    first,
    with_altrep(FALSE, charr:::ci_extract_first_fixed(values, patterns))
  )
...
test_that("fixed extraction covers aliases and matcher lanes", {
  ...
  actual <- with_altrep(TRUE, charr:::ci_extract_first_fixed(alias, alias))
  expect_true(charport::is_charvec(actual))
  expect_identical(
    actual,
    with_altrep(
      FALSE,
      charr:::ci_extract_first_fixed(alias_values, alias_values)
    )
  )
  ...
  pattern <- charport::as_charvec(patterns)
  expect_identical(
    with_altrep(TRUE, charr:::ci_extract_first_fixed(subject, pattern)),
    with_altrep(FALSE, charr:::ci_extract_first_fixed(subjects, patterns))
  )
  expect_fixed_output_input_unmaterialized(subject)
  expect_fixed_output_input_unmaterialized(pattern)
})
...
test_that("fixed extraction and replacement preserve case-insensitive matching", {
  ...
  expect_false(charport::charport_info(replaced)$is_materialized)
  expect_identical(
    replaced,
    with_altrep(
      FALSE,
...
test_that("fixed extract all keeps growable outputs lazy and chains", {
  ...
  expect_true(all(vapply(
    actual,
    function(x) !charport::charport_info(x)$is_materialized,
    logical(1)
  )))
  chained <- with_altrep(
    TRUE,
    charr:::ci_count_fixed(actual[[1]], charport::as_charvec("a"))
  )
...
test_that("fixed extraction preserves warning order and zero-length shapes", {
  ...
  first <- with_altrep(
    TRUE,
    charr:::ci_extract_first_fixed(zero_subject, zero_pattern)
  )
  all <- with_altrep(
    TRUE,
    charr:::ci_extract_all_fixed(zero_subject, zero_pattern)
  )
  simplified <- with_altrep(
    TRUE,
    charr:::ci_extract_all_fixed(
  ...
  expect_true(charport::is_charvec(simplified))
  expect_false(charport::charport_info(simplified)$is_materialized)
...
test_that("fixed extraction preserves BOM, malformed UTF-8, and byte errors", {
  ...
  actual <- with_altrep(TRUE, charr:::ci_extract_first_fixed(subject, pattern))
  expected <- with_altrep(
    FALSE,
    charr:::ci_extract_first_fixed(c(bom, malformed), c("a", bad_pattern))
  )
  expect_identical(actual, expected)
  ...
  expect_error(
    with_altrep(TRUE, charr:::ci_extract_first_fixed(bytes, "x")),
    "bytes encoding"
  )
  expect_error(
    with_altrep(TRUE, charr:::ci_extract_first_fixed("x", bytes)),
    "bytes encoding"
  )
})
...
test_that("fixed first and all replacement produce lazy outputs", {
  ...
  cases <- list(
    first = list(
      actual = with_altrep(
        TRUE,
        charr:::ci_replace_first_fixed(subject, pattern, replacement)
      ),
      expected = with_altrep(
        FALSE,
        charr:::ci_replace_first_fixed(values, patterns, replacements)
      )
    ),
    all = list(
      actual = with_altrep(
        TRUE,
        charr:::ci_replace_all_fixed(subject, pattern, replacement)
      ),
      expected = with_altrep(
        FALSE,
        charr:::ci_replace_all_fixed(values, patterns, replacements)
      )
    )
  )
  for (case in cases) {
    expect_true(charport::is_charvec(case$actual))
    expect_false(charport::charport_info(case$actual)$is_materialized)
...
test_that("fixed replacement preserves aliases, chaining, and matcher lanes", {
  ...
  actual <- with_altrep(
    TRUE,
    charr:::ci_replace_first_fixed(alias, alias, alias)
  )
  expect_true(charport::is_charvec(actual))
  expect_false(charport::charport_info(actual)$is_materialized)
  ...
  extracted <- with_altrep(
    TRUE,
    charr:::ci_extract_first_fixed(replaced, replacement)
  )
...
test_that("fixed replacement keeps missing replacements lazy", {
  ...
  expected <- with_altrep(
    FALSE,
    charr:::ci_replace_first_fixed(values, "a", NA_character_)
  )
  expect_identical(actual, expected)
...
test_that("sequential fixed replacement retains mutable-container semantics", {
  ...
  expect_true(charport::is_charvec(fast))
  expect_false(charport::charport_info(fast)$is_materialized)
...
test_that("fixed replacement zero recycling skips Reader acquisition", {
  ...
  actual <- with_altrep(
    TRUE,
    charr:::ci_replace_all_fixed(subject, "a", replacement)
  )
  expected <- with_altrep(
    FALSE,
    charr:::ci_replace_all_fixed(bytes, "a", character())
  )
  expect_true(charport::is_charvec(actual))
  expect_identical(actual, expected)
  ...
  expect_true(charport::is_charvec(sequential))
