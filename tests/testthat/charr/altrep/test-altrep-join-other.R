...
test_that("duplicate uses unmaterialized input and lazy output", {
  ...
  actual <- with_altrep(TRUE, charr:::ci_dup(subject, times))
  ...
  chained <- with_altrep(TRUE, charr:::ci_length(actual))
...
test_that("join preserves aliases, recycling, and collapse", {
  ...
  joined <- with_altrep(TRUE, charr:::ci_c(shared, shared))
  ...
  actual <- join_other_warnings(
    with_altrep(TRUE, charr:::ci_c(shared, right, sep = "|"))
  )
  ...
  collapsed <- suppressWarnings(
    with_altrep(
      TRUE,
      charr:::ci_c(shared, right, sep = "|", collapse = ";")
    )
  )
...
test_that("join inspects scalar separators without materializing them", {
  ...
  joined <- with_altrep(
    TRUE,
    charr:::ci_c(left, right, third, sep = sep, collapse = collapse)
  )
  expect_identical(
    joined,
    stringi::stri_c(
  ...
  uncollapsed <- with_altrep(
    TRUE,
    charr:::ci_c(left, right, sep = no_collapse_sep)
  )
  expect_identical(
    uncollapsed,
  ...
  flattened <- with_altrep(
    TRUE,
    charr:::ci_flatten(left, empty_collapse)
  )
  expect_identical(
    flattened,
...
test_that("join keeps copied early-return order", {
  ...
  empty_dup <- with_altrep(TRUE, charr:::ci_dup(character(), 2L))
  expect_join_other_unmaterialized(empty_dup)
  expect_identical(empty_dup, stringi::stri_dup(character(), 2L))

  empty_join <- with_altrep(TRUE, charr:::ci_c(subject, character()))
  ...
  ignored_sep <- with_altrep(
    TRUE,
    charr:::ci_c("x", sep = bytes, collapse = ",")
  )
  ...
  missing <- with_altrep(
    TRUE,
    charr:::ci_flatten(subject, NA_character_)
  )
...
test_that("line splitting recognizes every copied separator", {
  ...
  actual <- charr:::ci_split_lines(subject, FALSE)
...
test_that("line splitting preserves omit-empty recycling and encodings", {
  ...
  actual <- join_other_warnings(charr:::ci_split_lines(subject, omit))
  expect_identical(actual, expected)
  expect_join_other_unmaterialized(subject)
})
...
test_that("join and line splitting preserve malformed and bytes handling", {
  ...
  joined <- with_altrep(TRUE, charr:::ci_c(malformed_subject, "x"))
  expect_identical(
    charToRaw(joined),
    charToRaw(stringi::stri_c(malformed, "x"))
  )
  split <- charr:::ci_split_lines(malformed_subject, FALSE)
  expect_identical(split, stringi::stri_split_lines(malformed, FALSE))

  bytes <- join_other_marked(c(0xff, 0xfe), "bytes")
  bytes_subject <- charport::as_charvec(bytes)
  expect_error(
    with_altrep(TRUE, charr:::ci_dup(bytes_subject, 2L)),
    "bytes encoding"
  )
  expect_error(
    with_altrep(TRUE, charr:::ci_c(bytes_subject, "x")),
    "bytes encoding"
  )
  expect_error(
    with_altrep(TRUE, charr:::ci_flatten(bytes_subject, ",")),
    "bytes encoding"
  )
  expect_error(charr:::ci_split_lines(bytes_subject), "bytes encoding")
})
...
test_that("join emits list recycling warning before a bytes error", {
  ...
  actual <- join_other_events(
    with_altrep(
      TRUE,
      charr:::ci_c(bytes_subject, subject2, subject3, sep = "-")
    )
  )
  expect_identical(actual, expected)
  expect_identical(
