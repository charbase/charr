...
test_that("boundary character results preserve every iterator traversal", {
  ...
  for (case in cases) {
    ...
    first <- with_test_backend(
      TRUE,
      charr_test_leaf("ci_extract_first_boundaries")(
    ...
    all <- with_test_backend(
      TRUE,
      charr_test_leaf("ci_extract_all_boundaries")(
        subject, opts_brkiter = case$opts
      )
    )
    expect_identical(
...
test_that("boundary character results preserve skip rules and custom rules", {
  ...
  for (opts in option_cases) {
    ...
    split <- with_test_backend(
      TRUE,
      charr_test_leaf("ci_split_boundaries")(subject, opts_brkiter = opts)
    )
    expect_identical(
      split,
  ...
  custom_extract <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_extract_all_boundaries")(
  ...
  custom_split <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_split_boundaries")(
      custom_subject, opts_brkiter = custom_opts
    )
...
test_that("boundary extraction preserves NA, omit, and simplify shapes", {
  ...
  empty_first <- with_test_backend(
    TRUE, charr_test_leaf("ci_extract_first_boundaries")(empty, opts_brkiter = words)
  )
  empty_all <- with_test_backend(
    TRUE, charr_test_leaf("ci_extract_all_boundaries")(empty, opts_brkiter = words)
  )
  empty_matrix <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_extract_all_boundaries")(
...
test_that("boundary character output can be consumed without materializing", {
  ...
  extracted <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_extract_all_boundaries")(subject, opts_brkiter = words)
  )
  expect_boundary_char_list_unmaterialized(extracted)
...
test_that("boundary iterators preserve lazy-open warning and error order", {
  ...
  calls <- list(
    list(
      actual = quote(with_test_backend(
        TRUE,
        charr_test_leaf("ci_extract_first_boundaries")(
...
test_that("boundary character results preserve marked input semantics", {
  ...
  first <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_extract_first_boundaries")(subject, opts_brkiter = opts)
  )
  ...
  all <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_extract_all_boundaries")(subject, opts_brkiter = opts)
  )
  expect_identical(
    all,
