...
test_that("pad keeps all sides on unmaterialized inputs and outputs", {
  ...
  operations <- list(
    left = charr_test_leaf("ci_pad_left"),
    right = charr_test_leaf("ci_pad_right"),
    both = charr_test_leaf("ci_pad_both")
  )
...
test_that("pad output remains lazy when consumed by another pad call", {
  ...
  first <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_pad_left")(subject, 3L, first_pad, use_length = TRUE)
  )
  expect_pad_unmaterialized(first)
  second <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_pad_right")(first, 5L, second_pad, use_length = TRUE)
  )
  ...
  expected <- with_test_backend(
    ...
    charr_test_leaf("ci_pad_right")(
      charr_test_leaf("ci_pad_left")(values, 3L, ".", use_length = TRUE),
      5L, "-", use_length = TRUE
    )
  )
...
test_that("pad shares exact input aliases and preserves width semantics", {
  ...
  actual <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_pad_both")(shared, 3L, shared, use_length = TRUE)
  )
  expect_pad_unmaterialized(actual)
  expect_identical(
    actual,
    with_test_backend(
      FALSE,
      charr_test_leaf("ci_pad_both")(values, 3L, values, use_length = TRUE)
    )
  )
  ...
  expect_error(
    with_test_backend(
      TRUE,
      charr_test_leaf("ci_pad_left")(subject, 3L, wide_pad, use_length = FALSE)
    ),
    "total width 1"
  )
  wide <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_pad_left")(subject, 3L, wide_pad, use_length = TRUE)
  )
...
test_that("pad preserves malformed UTF-8 and bytes errors", {
  ...
  expect_error(
    with_test_backend(
      TRUE,
      charr_test_leaf("ci_pad_left")(bytes, 3L, valid_padding, use_length = TRUE)
    ),
    "bytes encoding"
  )
  expect_error(
    with_test_backend(
      TRUE,
      charr_test_leaf("ci_pad_left")(subject, 3L, bytes, use_length = TRUE)
    ),
    "bytes encoding"
  )
