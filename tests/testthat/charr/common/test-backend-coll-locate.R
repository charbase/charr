...
test_that("collation locate converts UTF-16 offsets to code-point positions", {
  ...
  first <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_locate_first_coll")(
  ...
  for (omit_no_match in c(FALSE, TRUE)) {
    for (get_length in c(FALSE, TRUE)) {
      actual <- with_test_backend(
        TRUE,
        charr_test_leaf("ci_locate_all_coll")(
          subject, pattern,
...
test_that("collation locate supports exact aliases and recycling", {
  ...
  expect_identical(
    with_test_backend(TRUE, charr_test_leaf("ci_locate_first_coll")(shared, shared)),
    stringi::stri_locate_first_coll(shared_values, shared_values)
  )
  expect_identical(
    with_test_backend(TRUE, charr_test_leaf("ci_locate_all_coll")(shared, shared)),
...
test_that("collation locate preserves marked and malformed inputs", {
  ...
  first <- with_test_backend(
    TRUE, charr_test_leaf("ci_locate_first_coll")(subject, pattern)
  )
  expect_identical(
    first,
  ...
  all <- with_test_backend(
    TRUE, charr_test_leaf("ci_locate_all_coll")(subject, pattern)
  )
  expect_identical(
    all,
...
test_that("collation locate preserves zero recycling and bytes errors", {
  ...
  first_empty <- with_test_backend(
    TRUE, charr_test_leaf("ci_locate_first_coll")(bytes_input, empty_pattern)
  )
  all_empty <- with_test_backend(
    TRUE, charr_test_leaf("ci_locate_all_coll")(bytes_input, empty_pattern)
  )
  expect_identical(
    first_empty,
  ...
  expect_error(
    with_test_backend(TRUE, charr_test_leaf("ci_locate_first_coll")(bytes_input, "x")),
    "bytes encoding"
  )
  expect_error(
    with_test_backend(TRUE, charr_test_leaf("ci_locate_first_coll")("x", bytes_input)),
    "bytes encoding"
  )
  expect_error(
    with_test_backend(TRUE, charr_test_leaf("ci_locate_all_coll")(bytes_input, "x")),
    "bytes encoding"
  )
  expect_error(
    with_test_backend(TRUE, charr_test_leaf("ci_locate_all_coll")("x", bytes_input)),
    "bytes encoding"
  )
