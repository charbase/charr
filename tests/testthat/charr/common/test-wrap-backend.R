# Charr-owned cross-backend tests for wrapping.

wrap_backend <- function(...) charr_test_leaf("ci_wrap")(...)

wrap_input <- function(x) {
...
test_that("wrap retains line breaking and output shapes", {
  family_text <- paste0(
    "\U0001f469\u200d\U0001f469\u200d\U0001f467\u200d\U0001f466",
    " e\u0301 family"
  )
  strings <- wrap_input(c(
  ...
  expect_identical(paste(got[[4L]], collapse = " "), family_text)
  expect_identical(
...
test_that("wrap preserves NA, empty, prefix, and initial shapes", {
  strings <- wrap_input(c("", " ", NA, "one two"))
  prefix <- wrap_input("> ")
  initial <- wrap_input("* ")
  expect_identical(charport::is_charvec(strings), charr_altrep())
...
test_that("wrap retains copied malformed UTF-8 and newline errors", {
  ...
  malformed <- wrap_input(malformed)
  newline <- wrap_input("alpha\nbeta")
...
test_that("wrap matches stringi over 500 seeded greedy and dynamic cases", {
  ...
  for (i in seq_len(250L)) {
    ...
    strings <- wrap_input(raw_strings)
    prefix <- wrap_input(sample(prefixes, 1L))
    initial <- wrap_input(sample(initials, 1L))
...
test_that("normalize strips U+FEFF at field starts like per-field flatten", {
  ...
  expect_identical(charport::is_charvec(strings), charr_altrep())
  expect_altrep_unmaterialized(strings)
  got <- wrap_backend(strings, width = 80L)
  expect_altrep_unmaterialized(got)
  expect_altrep_unmaterialized(strings)
