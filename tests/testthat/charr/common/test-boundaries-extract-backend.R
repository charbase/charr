# Charr-owned cross-backend tests for boundary extraction.

extract_first_boundaries <- function(...) charr_test_leaf("ci_extract_first_boundaries")(...)
extract_all_boundaries <- function(...) charr_test_leaf("ci_extract_all_boundaries")(...)

boundary_extract_input <- function(x) charr::str_trim(x)


test_that("boundary extract handles every ICU iterator type", {
  strings <- boundary_extract_input(c(
...
test_that("boundary extract honors skip rule-status ranges", {
  string <- boundary_extract_input("abc 123 日本語 カナ")
...
test_that("boundary extract preserves NA, no-match, omit, and simplify shapes", {
  strings <- boundary_extract_input(c("", NA, "abc"))
  expect_identical(charport::is_charvec(strings), charr_altrep())
  ...
  padded_empty <- extract_all_boundaries(
    boundary_extract_input(c("abc 123", "abc")), simplify = TRUE,
    opts_brkiter = list(type = "word", skip_word_none = TRUE)
  )
  padded_na <- extract_all_boundaries(
    boundary_extract_input(c("abc 123", "abc")), simplify = NA,
    opts_brkiter = list(type = "word", skip_word_none = TRUE)
  )
...
test_that("boundary extract supports custom RBBI rules", {
  ...
  strings <- boundary_extract_input(c("abc 123", "é42"))
  expect_identical(charport::is_charvec(strings), charr_altrep())
  expect_identical(
