# Charr-owned cross-backend tests for regex detection. Installed stringi is
# the oracle for the selected backend.

detect_regex <- function(...) charr_test_leaf("ci_detect_regex")(...)
...
test_that("regex detect preserves recycling, NA, zero-length, and max_count", {
  strings <- charr_test_leaf("ci_trim_both")(c(" a ", " b ", " 😀 ", "", NA, " aa "))
  expect_identical(charport::is_charvec(strings), charr_altrep())
...
test_that("regex detect preserves eager encoding and lazy compile errors", {
  ...
  exhausted <- charr_test_leaf("ci_trim_both")(c(" x ", " x "))
  ...
  bytes <- charr_test_leaf("ci_conv")("Ą", "UTF-8", "ISO-8859-2")
