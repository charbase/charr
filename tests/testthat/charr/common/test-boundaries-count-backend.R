# Charr-owned cross-backend tests for ICU boundary counting.

count_boundaries <- function(...) charr_test_leaf("ci_count_boundaries")(...)

boundary_count_input <- function(x) charr::str_trim(x)


test_that("boundary count handles every iterator type", {
  strings <- boundary_count_input(c(
...
test_that("boundary count keeps dictionary engines and custom rules", {
  dictionary <- boundary_count_input(c(
    "日本語の文章です", "ภาษาไทยภาษาไทย"
  ))
  ...
  custom <- boundary_count_input(c("abc 123", "é42"))
  expect_identical(
