...
dup_strings <- function(x, times) charr_test_leaf("ci_dup")(x, times)
join_strings <- function(..., sep = "", collapse = NULL, ignore_null = FALSE) {
  charr_test_leaf("ci_c")(..., sep = sep, collapse = collapse, ignore_null = ignore_null)
}
flatten_strings <- function(x, collapse = "", na_empty = FALSE,
                            omit_empty = FALSE) {
  charr_test_leaf("ci_flatten")(x, collapse, na_empty, omit_empty)
}
join_two <- function(x, y) charr_test_leaf("ci_c")(x, y)


test_that("dup preserves recycling and times edge cases", {
  strings <- charr_test_leaf("ci_trim_both")(c(" aé🙂 ", " x ", " ", NA))
...
test_that("binary and variadic joins recycle Reader records exactly", {
  left <- charr_test_leaf("ci_trim_both")(c(" a ", " é ", NA, " z "))
  right <- charr_test_leaf("ci_trim_both")(c(" 🙂 ", " x "))
...
test_that("join keeps empty-vector and collapsed special cases", {
  strings <- charr_test_leaf("ci_trim_both")(c(" a ", " é ", " 🙂 "))
  expect_identical(charport::is_charvec(strings), charr_altrep())
...
test_that("join copies malformed declared UTF-8 without validating", {
  ...
  malformed <- charr_test_leaf("ci_replace_all_fixed")("a", "a", malformed_byte)
