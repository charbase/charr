...
test_that("regex extraction keeps inputs and fixed outputs unmaterialized", {
  ...
  first <- with_test_backend(
    TRUE, charr_test_leaf("ci_extract_first_regex")(subject, pattern)
  )
  ...
  aliased <- with_test_backend(
    TRUE, charr_test_leaf("ci_extract_first_regex")(shared, shared)
  )
...
test_that("regex extract all preserves zero matches, omit, and simplify", {
  ...
  actual <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_extract_all_regex")(
  ...
  chained <- with_test_backend(
    TRUE, charr_test_leaf("ci_count_regex")(actual[[1]], charport::as_charvec("a"))
  )
  expect_identical(chained, rep(1L, 4L))

  zero <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_extract_all_regex")(
  ...
  for (simplify in list(TRUE, NA)) {
    matrix_result <- with_test_backend(
      TRUE,
      charr_test_leaf("ci_extract_all_regex")(
...
test_that("regex split preserves state transitions and lazy result shapes", {
  ...
  for (args in cases) {
    actual <- with_test_backend(
      TRUE,
      do.call(
        charr_test_leaf("ci_split_regex"),
  ...
  shared_values <- c("same", "two", "caf\u00e9")
  shared <- charport::as_charvec(shared_values)
  aliased <- with_test_backend(TRUE, charr_test_leaf("ci_split_regex")(shared, shared))
  expect_identical(
    aliased,
...
test_that("regex UTF-8 paths preserve Latin-1, BOM, malformed, and bytes", {
  ...
  split <- with_test_backend(
    TRUE, charr_test_leaf("ci_split_regex")(subject, delimiter)
  )
  expect_altrep_unmaterialized_list(split)
  ...
  extracted <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_extract_first_regex")(
  ...
  expect_error(
    with_test_backend(TRUE, charr_test_leaf("ci_extract_first_regex")(bytes_input, "x")),
    "bytes encoding"
  )
  expect_error(
    with_test_backend(TRUE, charr_test_leaf("ci_extract_first_regex")("x", bytes_input)),
    "bytes encoding"
  )
  expect_error(
    with_test_backend(TRUE, charr_test_leaf("ci_split_regex")(bytes_input, "x")),
    "bytes encoding"
  )
  expect_error(
    with_test_backend(TRUE, charr_test_leaf("ci_split_regex")("x", bytes_input)),
    "bytes encoding"
  )
...
test_that("regex character operations keep compilation lazy and event order", {
  ...
  expect_identical(
    with_test_backend(TRUE, charr_test_leaf("ci_extract_first_regex")(missing, invalid)),
    stringi::stri_extract_first_regex(NA_character_, "[")
  )
  expect_identical(
    with_test_backend(TRUE, charr_test_leaf("ci_extract_all_regex")(missing, invalid)),
    stringi::stri_extract_all_regex(NA_character_, "[")
  )
  expect_identical(
    with_test_backend(TRUE, charr_test_leaf("ci_split_regex")(missing, invalid)),
    stringi::stri_split_regex(NA_character_, "[")
  )
  expect_error(
    with_test_backend(TRUE, charr_test_leaf("ci_extract_first_regex")(present, invalid)),
    "U_REGEX_MISSING_CLOSE_BRACKET"
  )
  expect_error(
    with_test_backend(TRUE, charr_test_leaf("ci_split_regex")(present, invalid)),
    "U_REGEX_MISSING_CLOSE_BRACKET"
  )
  values <- c("x", "x", "x")
  patterns <- c("", "[")
  ...
  expect_identical(
    ...
    regex_char_events(
      with_test_backend(TRUE, charr_test_leaf("ci_split_regex")(subject, pattern))
    )
  )
