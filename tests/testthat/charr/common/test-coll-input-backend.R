...
detect_coll <- function(...) charr_test_leaf("ci_detect_coll")(...)
count_coll <- function(...) charr_test_leaf("ci_count_coll")(...)
starts_coll <- function(...) charr_test_leaf("ci_startswith_coll")(...)
ends_coll <- function(...) charr_test_leaf("ci_endswith_coll")(...)

coll_input <- function(x) charr_test_leaf("ci_trim_both")(x)


test_that("coll input search reads CHARVEC UTF-16 records", {
  strings <- coll_input(c(
    " äpfel ", " Apfel ", " blåbær ", " blaa ", " 😀ä ", " ", NA
  ))
  patterns <- coll_input(c("a", "a", "blaa", "blå", "ä", "x", "a"))
  ...
  expect_identical(
    detect_coll(strings, coll_input("A"), opts_collator = tertiary),
...
test_that("coll starts and ends use code-point from/to indexes", {
  strings <- coll_input(rep("aé😀bcä", 8L))
  patterns <- coll_input(c("a", "é", "😀", "b", "c", "a", "ä", "x"))
  expect_identical(charport::is_charvec(strings), charr_altrep())
  expect_identical(charport::is_charvec(patterns), charr_altrep())
...
test_that("coll UTF-16 cursor recovers after missing subjects", {
  strings <- coll_input(c(NA, "Äpfel", "", "Apfel"))
  pattern <- coll_input("apfel")
  opts <- list(locale = "de", strength = 1L)
...
test_that("coll empty patterns warn once and numeric search errors", {
  strings <- coll_input(c("abc", "", NA))
  empty <- coll_input("")
  numeric_pattern <- coll_input("x2")
  expect_identical(charport::is_charvec(strings), charr_altrep())
  ...
  expect_error(
    detect_coll(coll_input("x02"), numeric_pattern,
