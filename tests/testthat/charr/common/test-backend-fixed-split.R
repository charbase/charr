...
test_that("fixed split returns unmaterialized fields and supports chaining", {
  ...
  actual <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_split_fixed")(subject, pattern)
  )
  ...
  expect_identical(
    with_test_backend(TRUE, charr_test_leaf("ci_count_fixed")(actual[[2]], "|")),
    integer(3)
  )
...
test_that("fixed split handles aliases and every matcher lane", {
  ...
  expect_identical(
    with_test_backend(TRUE, charr_test_leaf("ci_split_fixed")(subject, pattern)),
  ...
  expect_identical(
    with_test_backend(TRUE, charr_test_leaf("ci_split_fixed")(shared, shared)),
...
test_that("fixed split preserves simplify values and zero shapes", {
  ...
  expect_identical(
    expect_no_warning(
      with_test_backend(
        TRUE,
        charr_test_leaf("ci_split_fixed")(bytes_subject, empty_pattern)
      )
    ),
    list()
  )
...
test_that("fixed split preserves recycling and empty-pattern warnings", {
  ...
  actual <- capture_fixed_split_warnings(
    with_test_backend(
      TRUE,
      charr_test_leaf("ci_split_fixed")(subject, empty_pattern)
    )
  )
  expect_identical(actual, expected)
  expect_fixed_split_unmaterialized(subject)
...
test_that("fixed split normalizes Latin-1 and BOM inputs", {
  ...
  actual <- with_test_backend(TRUE, charr_test_leaf("ci_split_fixed")(subject, pattern))
...
test_that("fixed split preserves malformed UTF-8 and long field runs", {
  ...
  fields <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_split_fixed")(malformed_subject, "|")
  )[[1]]
  ...
  actual <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_split_fixed")(subject, "|", omit_empty = NA)
  )
  expect_altrep_charvec_list(actual)
  expect_identical(
    actual,
...
test_that("fixed split rejects bytes after preserving zero recycling", {
  ...
  expect_error(
    with_test_backend(TRUE, charr_test_leaf("ci_split_fixed")(bytes_subject, plain_pattern)),
    "bytes encoding"
  )
  expect_error(
    with_test_backend(TRUE, charr_test_leaf("ci_split_fixed")(plain_subject, bytes_pattern)),
    "bytes encoding"
  )
  expect_fixed_split_unmaterialized(bytes_subject)
