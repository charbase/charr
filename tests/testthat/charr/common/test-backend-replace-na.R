...
test_that("replace_na returns unmaterialized normalized output", {
  ...
  actual <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_replace_na")(subject, replacement)
  )
  expected <- with_test_backend(
    FALSE,
    charr_test_leaf("ci_replace_na")(values, replacement_value)
  )
  expect_replace_na_unmaterialized(actual)
  expect_identical(actual, expected)
  expect_identical(
    with_test_backend(TRUE, charr_test_leaf("ci_count_fixed")(actual, "\u00e9")),
...
test_that("replace_na handles missing replacement and exact aliases", {
  ...
  expected <- with_test_backend(
    FALSE,
    charr_test_leaf("ci_replace_na")(values, NA_character_)
  )
  expect_identical(actual, expected)
  expect_replace_na_unmaterialized(actual)
  expect_replace_na_unmaterialized(subject)
  ...
  expect_identical(
    with_test_backend(TRUE, charr_test_leaf("ci_replace_na")(shared, shared)),
    with_test_backend(
      FALSE,
      charr_test_leaf("ci_replace_na")(NA_character_, NA_character_)
    )
  )
...
test_that("replace_na validates bytes eagerly in copied container order", {
  ...
  empty_result <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_replace_na")(empty_subject, "x")
  )
  ...
  expect_error(
    with_test_backend(
      TRUE,
      charr_test_leaf("ci_replace_na")(bytes_subject, "x")
    ),
    "bytes encoding"
  )
  expect_error(
    with_test_backend(
      TRUE,
      charr_test_leaf("ci_replace_na")(plain_subject, bytes_replacement)
    ),
    "bytes encoding"
  )
  expect_error(
    with_test_backend(
      TRUE,
      charr_test_leaf("ci_replace_na")(empty_subject, bytes_replacement)
    ),
    "bytes encoding"
  )
...
test_that("replace_na preserves scalar replacement preparation", {
  ...
  expect_warning(
    expected <- with_test_backend(
      FALSE,
      charr_test_leaf("ci_replace_na")(values, replacement_values)
    ),
    warning
  )
  expect_warning(
  ...
  expect_error(
    with_test_backend(TRUE, charr_test_leaf("ci_replace_na")(subject, character())),
