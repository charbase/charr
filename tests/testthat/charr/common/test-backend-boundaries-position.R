...
boundary_position_count <- function(...) {
  charr_test_leaf("ci_count_boundaries")(...)
}

boundary_position_first <- function(...) {
  charr_test_leaf("ci_locate_first_boundaries")(...)
}

boundary_position_all <- function(...) {
  charr_test_leaf("ci_locate_all_boundaries")(...)
}
...
test_that("boundary counts consume direct and chained unmaterialized inputs", {
  ...
  chained <- with_test_backend(TRUE, str_trim(direct))
  option_cases <- list(
    list(type = "character"),
...
test_that("boundary locate preserves code-point positions and iterator rules", {
  ...
  emoji <- with_test_backend(TRUE, str_trim(emoji_direct))
...
test_that("boundary positions preserve encoding normalization and bytes errors", {
  ...
  expect_error(
    with_test_backend(TRUE, boundary_position_count(bytes_input)),
    "bytes encoding"
  )
  expect_error(
    with_test_backend(TRUE, boundary_position_first(bytes_input)),
    "bytes encoding"
  )
  expect_error(
    with_test_backend(TRUE, boundary_position_all(bytes_input)),
    "bytes encoding"
  )
