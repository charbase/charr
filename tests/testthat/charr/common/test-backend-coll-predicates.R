...
test_that("collation detect and count consume unmaterialized UTF-16 inputs", {
  ...
  expect_identical(
    with_test_backend(
      TRUE,
      charr_test_leaf("ci_detect_coll")(subject, pattern, opts_collator = primary)
    ),
  ...
  expect_identical(
    with_test_backend(
      TRUE,
      charr_test_leaf("ci_detect_coll")(subject, "A", opts_collator = tertiary)
    ),
  ...
  expect_identical(
    with_test_backend(
      TRUE,
      charr_test_leaf("ci_count_coll")(subject, pattern, opts_collator = primary)
    ),
...
test_that("collation predicates accept an exact subject-pattern alias", {
  ...
  expect_identical(
    with_test_backend(
      TRUE,
      charr_test_leaf("ci_detect_coll")(shared, shared, opts_collator = opts)
    ),
  ...
  expect_identical(
    with_test_backend(
      TRUE,
      charr_test_leaf("ci_count_coll")(shared, shared, opts_collator = opts)
    ),
  ...
  expect_identical(
    with_test_backend(
      TRUE,
      charr_test_leaf("ci_startswith_coll")(shared, shared, opts_collator = opts)
    ),
  ...
  expect_identical(
    with_test_backend(
      TRUE,
      charr_test_leaf("ci_endswith_coll")(shared, shared, opts_collator = opts)
    ),
  ...
  for (negate in c(FALSE, TRUE)) {
    for (max_count in c(-1L, 0L, 2L)) {
      expect_identical(
        with_test_backend(
          TRUE,
          charr_test_leaf("ci_detect_coll")(
...
test_that("collation counting keeps UStringSearch match advancement", {
  ...
  expect_identical(
    with_test_backend(
      TRUE,
      charr_test_leaf("ci_count_coll")(subject, pattern, opts_collator = opts)
    ),
...
test_that("collation starts and ends use code-point indexes", {
  ...
  for (negate in c(FALSE, TRUE)) {
    expect_identical(
      with_test_backend(
        TRUE,
        charr_test_leaf("ci_startswith_coll")(
    ...
    expect_identical(
      with_test_backend(
        TRUE,
        charr_test_leaf("ci_endswith_coll")(
...
test_that("collation predicates consume a previous charvec result", {
  ...
  expect_identical(
    with_test_backend(
      TRUE,
      charr_test_leaf("ci_detect_coll")(subject, "a", opts_collator = opts)
    ),
    stringi::stri_detect_coll(
  ...
  for (call in calls) {
    expect_identical(
      coll_pred_condition_events(
        with_test_backend(TRUE, eval(call$actual))
      ),
...
test_that("empty-pattern warnings precede collation search errors", {
  ...
  expect_identical(
    coll_pred_condition_events(
      with_test_backend(
        TRUE,
        charr_test_leaf("ci_detect_coll")(subject, pattern, opts_collator = opts)
      )
    ),
    coll_pred_condition_events(
  ...
  expect_identical(
    coll_pred_condition_events(
      with_test_backend(
        TRUE,
        charr_test_leaf("ci_startswith_coll")(maxed_subject, empty_pattern)
      )
    ),
    coll_pred_condition_events(
  ...
  expect_identical(
    coll_pred_condition_events(
      with_test_backend(
        TRUE,
        charr_test_leaf("ci_endswith_coll")(maxed_subject, empty_pattern)
      )
    ),
    coll_pred_condition_events(
  ...
  expect_identical(
    coll_pred_condition_events(
      with_test_backend(TRUE, charr_test_leaf("ci_count_coll")("a", two_empty))
    ),
...
test_that("collation predicates eagerly reject bytes inputs", {
  ...
  cases <- list(
    ...
    list(
      actual = quote(charr_test_leaf("ci_count_coll")(plain_subject, bytes_pattern)),
...
test_that("empty collation inputs preserve zero-length recycling", {
  ...
  expect_identical(
    with_test_backend(TRUE, charr_test_leaf("ci_detect_coll")(subject, pattern)),
    stringi::stri_detect_coll(character(), "a")
  )
  expect_identical(
    with_test_backend(TRUE, charr_test_leaf("ci_count_coll")(subject, pattern)),
    stringi::stri_count_coll(character(), "a")
  )
  expect_identical(
    with_test_backend(TRUE, charr_test_leaf("ci_startswith_coll")(subject, pattern)),
    stringi::stri_startswith_coll(character(), "a")
  )
  expect_identical(
    with_test_backend(TRUE, charr_test_leaf("ci_endswith_coll")(subject, pattern)),
