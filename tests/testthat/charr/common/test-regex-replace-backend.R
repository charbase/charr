...
replace_all <- function(...) charr_test_leaf("ci_replace_all_regex")(...)
replace_first <- function(...) charr_test_leaf("ci_replace_first_regex")(...)


test_that("regex replace applies $-substitution over UTF-16 subjects", {
  strings <- charr_test_leaf("ci_trim_both")(c(" abcde ", " a1b2 ", " é😀x ", NA))
...
test_that("regex replace first and NA replacement preserve values", {
  strings <- charr_test_leaf("ci_replace_all_fixed")(c("ababab", "xyz", "aa"), "~", "~")
  expect_identical(charport::is_charvec(strings), charr_altrep())
...
test_that("regex replace raises ICU error on an invalid backreference", {
  strings <- charr_test_leaf("ci_replace_all_fixed")("abcde", "~", "~")
  expect_identical(charport::is_charvec(strings), charr_altrep())
...
test_that("regex replace vectorize_all = FALSE applies patterns in sequence", {
  strings <- charr_test_leaf("ci_replace_all_fixed")(c("a1b2", "cccc", NA), "~", "~")
  expect_identical(charport::is_charvec(strings), charr_altrep())
