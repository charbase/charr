...
test_that("regex replacement keeps the copied vectorized traversal", {
  ...
  actual_all <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_replace_all_regex")(subject, pattern, replacement)
  )
  ...
  actual_first <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_replace_first_regex")(subject, pattern, replacement)
  )
  ...
  chained <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_count_regex")(actual_all, charport::as_charvec("[#<]"))
  )
  expect_identical(
    chained,
...
test_that("regex replacement accepts exact aliases for all inputs", {
  ...
  actual <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_replace_all_regex")(shared, shared, shared)
  )
  expect_identical(
    actual,
...
test_that("regex replacement preserves captures and missing values", {
  ...
  expect_identical(
    with_test_backend(
      TRUE,
      charr_test_leaf("ci_replace_all_regex")(subject, "(a)", "[$0:$1]")
    ),
    stringi::stri_replace_all_regex(values, "(a)", "[$0:$1]")
  )
  expect_identical(
    with_test_backend(TRUE, charr_test_leaf("ci_replace_first_regex")(subject, "a", "Z")),
    stringi::stri_replace_first_regex(values, "a", "Z")
  )
  expect_identical(
    with_test_backend(
      TRUE,
      charr_test_leaf("ci_replace_all_regex")(subject, "a", NA_character_)
    ),
    stringi::stri_replace_all_regex(values, "a", NA_character_)
  )
  expect_identical(
    with_test_backend(TRUE, charr_test_leaf("ci_replace_all_regex")(subject, ".*", "x")),
...
test_that("non-vectorized regex replacement applies patterns in sequence", {
  ...
  missing_actual <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_replace_all_regex")(
...
test_that("regex compilation stays lazy on replacement paths", {
  ...
  expect_identical(
    with_test_backend(
      TRUE,
      charr_test_leaf("ci_replace_all_regex")(missing, invalid, replacement)
    ),
  ...
  expect_identical(
    regex_replace_condition_events(
      with_test_backend(
        TRUE,
        charr_test_leaf("ci_replace_all_regex")(present, invalid, replacement)
      )
    ),
    regex_replace_condition_events(
