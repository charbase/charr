...
test_that("character-class replace-all stays lazy and preserves vectorization", {
  ...
  all <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_replace_all_charclass")(
...
test_that("character-class trim routes through lazy backend output", {
  ...
  for (name in c("ci_trim_left", "ci_trim_right", "ci_trim_both")) {
    actual <- with_test_backend(
      TRUE,
      charr_test_leaf(name)(subject, pattern)
    )
    expected <- do.call(
  ...
  trimmed <- with_test_backend(TRUE, charr_test_leaf("ci_trim_both")(subject, pattern))
  expect_identical(
    with_test_backend(TRUE, charr_test_leaf("ci_length")(trimmed)),
  ...
  expect_error(
    with_test_backend(TRUE, charr_test_leaf("ci_trim_both")(edge_input)),
    "invalid UTF-8 byte sequence"
  )
  interior <- with_test_backend(TRUE, charr_test_leaf("ci_trim_both")(interior_input))
  ...
  expect_error(
    with_test_backend(TRUE, charr_test_leaf("ci_trim_both")(bytes_input)),
    "bytes encoding"
  )
  ...
  expect_error(
    with_test_backend(TRUE, charr_test_leaf("ci_trim_both")(class_input("x"), invalid_pattern)),
    "UnicodeSet pattern"
  )
  expect_class_unmaterialized(subject)
  expect_class_unmaterialized(pattern)
...
test_that("character-class trim preserves vectorized pattern recycling", {
  ...
  actual <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_trim_both")(subject, pattern)
  )
