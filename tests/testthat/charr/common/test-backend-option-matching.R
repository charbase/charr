...
test_that("break type reads scalar record zero when type is not first", {
  ...
  expect_identical(
    charr_test_leaf("ci_count_boundaries")("alpha beta", opts),
    2L
  )
  ...
  expect_identical(
    charr_test_leaf("ci_count_boundaries")("alpha beta", opts_brkiter = aliased),
    3L
  )
  ...
  expect_identical(
    option_events(charr_test_leaf("ci_count_boundaries")(
