...
expect_coll_character_list_unmaterialized <- function(x) {
  expect_altrep_unmaterialized_list(x)
}
...
test_that("collation character routes share exact aliases and chain outputs", {
  previous_backend <- charr_backend(selected_test_backend)
  on.exit(charr_backend(previous_backend), add = TRUE)

  values <- c("a", "b", "å")
  alias <- charport::as_charvec(values)
  opts <- list(locale = "en", strength = 3L)

  first <- charr_test_leaf("ci_extract_first_coll")(
    alias, alias, opts_collator = opts
  )
  all <- charr_test_leaf("ci_extract_all_coll")(
    alias, alias, opts_collator = opts
  )
  split <- charr_test_leaf("ci_split_coll")(
...
test_that("collation extraction reloads recycled subjects after slicing", {
  previous_backend <- charr_backend(selected_test_backend)
  on.exit(charr_backend(previous_backend), add = TRUE)

  subject <- charport::as_charvec("alpha beta")
  patterns <- charport::as_charvec(c("alpha", "beta"))
  actual <- charr_test_leaf("ci_extract_first_coll")(subject, patterns)

  expect_coll_character_unmaterialized(actual)
  expect_identical(
    actual,
    stringi::stri_extract_first_coll("alpha beta", c("alpha", "beta"))
  )
})
...
test_that("collation extract preserves tailored slices and output shapes", {
  ...
  omitted <- charr_test_leaf("ci_extract_all_coll")(
    subject, pattern, omit_no_match = TRUE,
    opts_collator = opts
  )
  simplified <- charr_test_leaf("ci_extract_all_coll")(
    subject, pattern, simplify = TRUE, opts_collator = opts
  )
  simplified_na <- charr_test_leaf("ci_extract_all_coll")(
...
test_that("collation split preserves fields, limits, and simplify padding", {
  ...
  all <- charr_test_leaf("ci_split_coll")(
    subject, pattern, opts_collator = opts
  )
  limited <- charr_test_leaf("ci_split_coll")(
    subject, pattern, n = 2L, opts_collator = opts
  )
  tokens <- charr_test_leaf("ci_split_coll")(
    subject, pattern, n = 2L, tokens_only = TRUE,
    opts_collator = opts
  )
  omit_na <- charr_test_leaf("ci_split_coll")(
    subject, pattern, n = 3L, omit_empty = NA,
    opts_collator = opts
  )
  simplified <- charr_test_leaf("ci_split_coll")(
    subject, pattern, n = 2L, simplify = TRUE,
    opts_collator = opts
  )
  simplified_na <- charr_test_leaf("ci_split_coll")(
...
test_that("collation character routes preserve input encoding behavior", {
  ...
  extracted <- charr_test_leaf("ci_extract_first_coll")(subject, pattern)
  split <- charr_test_leaf("ci_split_coll")(subject, pattern, n = 2L)
...
test_that("collation character warnings and errors match stringi", {
  ...
  expect_identical(
    ...
    coll_character_conditions(
      charr_test_leaf("ci_extract_all_coll")(subject, empty)
    )
  )
  expect_identical(
    coll_character_conditions(
    ...
    coll_character_conditions(
      charr_test_leaf("ci_split_coll")(subject, empty)
    )
  )
  ...
  expect_identical(
    ...
    coll_character_conditions(
      charr_test_leaf("ci_extract_first_coll")(
        "x02", "x2", opts_collator = numeric
      )
    )$error
  )
...
test_that("collation character routes reject bytes-marked records", {
  ...
  cases <- list(
    list(
      ...
      actual = function() charr_test_leaf("ci_extract_first_coll")(bytes_input, "x")
    ),
    list(
      oracle = function() stringi::stri_split_coll("x", bytes),
      actual = function() charr_test_leaf("ci_split_coll")("x", bytes_input)
    )
  )
