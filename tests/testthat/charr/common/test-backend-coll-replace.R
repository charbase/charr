...
test_that("collation replacement preserves all vectorized traversals", {
  ...
  actual_all <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_replace_all_coll")(
  ...
  actual_first <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_replace_first_coll")(
      subject, pattern, replacement, opts_collator = opts
    )
  )
  expect_identical(
...
test_that("collation replacement accepts aliases and sequential patterns", {
  ...
  aliased <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_replace_all_coll")(shared, shared, shared)
  )
  expect_identical(
    aliased,
...
test_that("collation replacement preserves validation and option order", {
  vectorized_prepare <- coll_replace_condition_events(
    with_test_backend(
      TRUE,
      charr_test_leaf("ci_replace_all_coll")("a", new.env(), new.env())
    )
  )
  expect_identical(
    vectorized_prepare,
...
test_that("collation replacement preserves marked and malformed strings", {
  ...
  actual <- with_test_backend(
    TRUE,
    charr_test_leaf("ci_replace_all_coll")(subject, pattern, replacement)
  )
