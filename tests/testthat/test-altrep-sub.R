...
test_that("positional extraction keeps zero and NA results Builder-backed", {
  ...
  empty <- with_altrep(
    TRUE,
    charr:::ci_sub(bytes_subject, integer(), integer())
  )
  empty_replacement <- with_altrep(
    TRUE,
  ...
  missing <- with_altrep(
    TRUE,
    charr:::ci_sub(charport::as_charvec("abc"), NA_integer_, 1L)
  )
...
test_that("sub_all converts only source records with requested substrings", {
  ...
  actual <- with_altrep(
    TRUE,
    charr:::ci_sub_all(subject, from = from, to = to)
  )
...
test_that("sub_all reuses one source borrow across many singleton records", {
  ...
  actual <- with_altrep(
    TRUE,
    charr:::ci_sub_all(subject, from = list(2L), to = list(2L))
  )
...
test_that("positional forms emit recycling warnings before bytes errors", {
  ...
  actual <- sub_condition_events(
    with_altrep(
      TRUE,
      charr:::ci_sub(subject, from = 1:2, to = 1:3)
    )
  )
  expect_identical(actual, expected)
  ...
  actual_all <- sub_condition_events(
    with_altrep(
      TRUE,
      charr:::ci_sub_all(all_subject, from = from, to = to)
    )
  )
