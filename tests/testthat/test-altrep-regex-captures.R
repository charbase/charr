...
test_that("regex locate and match accept one exact charvec alias", {
  ...
  located <- with_altrep(
    TRUE, charr:::ci_locate_first_regex(alias, alias)
  )
  matched <- with_altrep(
    TRUE, charr:::ci_match_first_regex(alias, alias)
  )
...
test_that("regex captures preserve values, names, and matrix attributes", {
  ...
  first_match <- with_altrep(
    TRUE,
    charr:::ci_match_first_regex(
      subject, pattern, cg_missing = "MISS"
    )
  )
  all_match <- with_altrep(
    TRUE,
    charr:::ci_match_all_regex(
      subject, pattern, cg_missing = "MISS"
    )
  )
  ...
  expect_true(all(vapply(
    all_match, charport::is_charvec, logical(1)
  )))
  expect_true(all(vapply(
    all_match,
...
test_that("vectorized regex patterns keep per-element capture metadata", {
  ...
  matched <- with_altrep(
    TRUE, charr:::ci_match_all_regex(subject, pattern)
  )
  ...
  expect_true(all(vapply(
    matched, charport::is_charvec, logical(1)
  )))
  expect_true(all(vapply(
    matched,
    function(x) !charport::charport_info(x)$is_materialized,
...
test_that("regex captures preserve empty-input and no-match shapes", {
  ...
  first <- with_altrep(
    TRUE, charr:::ci_match_first_regex(subject, pattern)
  )
  all <- with_altrep(
    TRUE, charr:::ci_match_all_regex(subject, pattern)
  )
...
test_that("regex captures preserve warnings, recycling, and lazy errors", {
  ...
  expect_identical(
    ...
    regex_capture_conditions(
      with_altrep(
        TRUE, charr:::ci_locate_all_regex(subject, empty_pattern)
      )
    )
  )
  expect_identical(
    ...
    regex_capture_conditions(
      with_altrep(
        TRUE, charr:::ci_match_first_regex(subject, empty_pattern)
      )
    )
  )
  ...
  expect_error(
    with_altrep(
      TRUE, charr:::ci_locate_first_regex(missing, invalid)
    ),
    "U_REGEX_MISSING_CLOSE_BRACKET"
  )
  expect_error(
  ...
  expect_error(
    with_altrep(
      TRUE, charr:::ci_match_first_regex(missing, invalid)
    ),
    "U_REGEX_MISSING_CLOSE_BRACKET"
  )
  expect_regex_capture_unmaterialized(subject)
...
test_that("regex captures preserve marked-input normalization", {
  ...
  expect_identical(
    with_altrep(
      TRUE, charr:::ci_locate_all_regex(subject, pattern)
    ),
    stringi::stri_locate_all_regex(values, locate_patterns)
  )

  first <- with_altrep(
    TRUE, charr:::ci_match_first_regex(subject, "(.)")
  )
  all <- with_altrep(
    TRUE, charr:::ci_match_all_regex(subject, "(.)")
  )
  expect_regex_capture_unmaterialized(first)
  expect_true(all(vapply(all, charport::is_charvec, logical(1))))
  expect_identical(
...
test_that("regex captures reject bytes-marked subjects and patterns", {
  ...
  expect_error(
    with_altrep(
      TRUE, charr:::ci_locate_all_regex(bytes_input, "x")
    ),
    "bytes encoding"
  )
  expect_error(
  ...
  expect_error(
    with_altrep(
      TRUE, charr:::ci_match_all_regex(bytes_input, "(.)")
    ),
    "bytes encoding"
  )
  expect_error(
  ...
  expect_error(
    with_altrep(
      TRUE, charr:::ci_match_first_regex("x", bytes_input)
    ),
    "bytes encoding"
  )
