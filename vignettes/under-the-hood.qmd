...
The benchmark scripts are in `tools/benchmark`.
...
On one thread, ALTREP is faster than the reference on all 67 operations, with medians from 1.04× to 13.9×. Five operations have no four-thread bar because they have no data-parallel shape: `str_flatten` reduces the whole vector to one string, `str_order`, `str_rank` and `str_unique` are whole-vector algorithms, and `str_read_lines` learns its record count only by reading. Across the other 62, four threads run from 1.31× to 27.9×, with a median of 4.60×.

Base medians run from 0.98× to 3.45×. Two sit below 1.00×, both trims: `str_trim (left)` at 0.99× is inside the reference's run-to-run spread, while `str_trim (both)` at 0.98× is not — its five repetitions span 200.6–202.9 ms against the reference's 196.8–198.0 ms, so it is about 2% behind rather than tied.
