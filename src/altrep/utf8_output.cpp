...
{
    return finalize(scalar_store(value));
}
...
{
    return finalize(scalar_store(data, length, encoding));
}

SEXP scalar_sexp(std::string_view value, cetype_ext_t encoding)
{
    return finalize(scalar_store(value, encoding));
}
