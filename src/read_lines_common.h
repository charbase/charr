#ifndef CHARR_READ_LINES_COMMON_H
#define CHARR_READ_LINES_COMMON_H
...
namespace charr { namespace read_lines {
...
class FileConditionError : public std::runtime_error {
private:
...
public:
    FileConditionError(FileCondition condition, int error)
        : std::runtime_error("cannot open the connection"),
...
inline int read_file_into(const char* path, Allocate allocate)
{
    ...
    if (static_cast<unsigned long long>(end) >
            static_cast<unsigned long long>(R_LEN_T_MAX))
...
)
{
    ...
    const size_t extra = invalid.size() >
        ...
        : invalid.size()*2U;
    if (extra > static_cast<size_t>(R_LEN_T_MAX)-static_cast<size_t>(length))
